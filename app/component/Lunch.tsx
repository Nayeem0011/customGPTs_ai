...
export default function LaunchSteps() {
  ...
  return (
    <section className="w-full flex flex-col items-center py-12 lg:py-20 px-4 sm:px-6 bg-[#eceef5]">

      {/* Heading */}
      <h2 className="text-center mb-3 text-[28px] sm:text-[36px] lg:text-[48px] font-medium leading-[130%] w-full max-w-[677px]" style={{ fontFamily: 'Geist' }}>
        <span className="text-[#1a1a2e]">Launch in 3 </span>
        <span className="text-[#9ba3c4]">streamlined steps</span>
      </h2>

      {/* Subtext */}
      <p className="text-center mb-8 font-normal text-[15px] sm:text-[18px] leading-[140%]" style={{ fontFamily: 'Geist', color: '#4a6bdc' }}>
        No coding required &nbsp;-&nbsp; No engineering costs
      </p>

      {/* Tabs */}
      <div className="flex mb-10 lg:mb-16 rounded-full bg-white border border-[#e0e2ed] overflow-hidden w-full max-w-[420px] sm:w-auto sm:max-w-none">
        {tabs.map((tab, i) => (
          <button
            key={tab}
            onClick={() => setActive(i)}
            className={`flex-1 sm:flex-none px-4 sm:px-9 py-3 sm:py-4 rounded-full text-[13px] sm:text-[16px] transition-all border-none cursor-pointer
              ${active === i
                ? 'bg-white text-[#1a1a2e] font-semibold shadow-md'
                : 'bg-transparent text-[#7b82a8] font-normal'
              }`}
      ...
      {/* Big Card */}
      <div
        className="w-full max-w-[1060px] rounded-2xl lg:rounded-3xl flex flex-col items-center pt-10 lg:pt-14 pb-8 lg:pb-10 px-4 sm:px-8 min-h-[480px] lg:min-h-[600px]"
        style={{
          background: 'linear-gradient(135deg, #c5cfe8 0%, #cec5e0 35%, #ddc5cc 65%, #c8cde8 100%)',
        }}
      >
        {/* Card Title */}
        <h3 className="text-[22px] sm:text-[28px] lg:text-[36px] font-semibold text-white text-center leading-[1.2] mb-3">
          Connect to all your knowledge<br className="hidden sm:block" />
          {" "}with 1-click integrations
        </h3>
        <p className="text-center text-[13px] sm:text-[15px] text-white/85 mb-8 lg:mb-10 max-w-[360px] sm:max-w-none">
          Make your information accessible with 1400+ supported
          <br className="hidden sm:block" />{" "}file types and dozens of integrations
        </p>
        ...
        <div className="w-full max-w-[440px] rounded-2xl overflow-hidden bg-white/90 backdrop-blur-md">
          ...
          {[
            ...
            <div
              key={item.label}
              className="flex items-center gap-3 mx-3 sm:mx-4 mb-2 px-3 sm:px-4 py-3 rounded-xl text-sm bg-[#f0f2fa]/70 text-[#2a2a3a] border border-[#b4b9d2]/20"
            >
              <span className="text-sm text-[#5a6080]">{item.icon}</span>
              {item.label}
            </div>
      ...
      {/* See All Button */}
      <button className="mt-6 lg:mt-8 px-6 sm:px-8 py-3 rounded-full text-sm font-medium bg-white border border-[#d0d4e8] text-[#3a3a5a] cursor-pointer hover:bg-gray-50 transition-colors">
        See all 100+ integrations
      </button>
